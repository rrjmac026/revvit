...
$quiz_questions = [
    [
        ...
        'options' => [
            'A' => 'Au revoir, comment allez-vous?',
            'B' => 'Bonjour, comment allez-vous?',
            'C' => 'Bonsoir, où allez-vous?',
            'D' => 'Salut, qui êtes-vous?'
        ],
        'correct_answer' => 'B',
        'explanation' => '"Bonjour, comment allez-vous?" is the formal way to say "Hello, how are you?" in French.'
    ],
    [
        ...
        'correct_answer' => 'A',
        'explanation' => '"Enchanté(e)" is used when meeting someone for the first time and means "Nice to meet you".'
    ],
    [
        'question' => 'How do you ask "Where is the restaurant?" in French?',
        'options' => [
            'A' => 'Quand est le restaurant?',
            'B' => 'Pourquoi est le restaurant?',
            'C' => 'Où est le restaurant?',
            'D' => 'Comment est le restaurant?'
        ],
        'correct_answer' => 'C',
        'explanation' => '"Où est le restaurant?" is the correct way to ask "Where is the restaurant?" in French.'
    ]
];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        // Debug output
        error_log("French Conversation Quiz - Subject/Course Query Results: " . print_r($subject_row, true));
        ...
        if ($subject_id && $course_id) {
            ...
            if (!$existing_enrollment) {
                ...
                if (!$stmt->execute()) {
                    ...
                    error_log("Successfully enrolled in French Conversation course. User: $user_id");
                }
            }
            ...
            if (!$stmt->execute()) {
                ...
                error_log("Successfully recorded French Conversation quiz attempt. User: $user_id, Score: $percentage%");
            }
        } else {
            error_log("Subject/Course ID not found for French Conversation quiz - Please check subjects and courses tables");
        }
    } catch (Exception $e) {
...
<head>
    ...
    <title>French Conversation Quiz | RevvIt</title>
...
<body class="bg-orange-50 min-h-screen flex flex-col">
    ...
    <div class="container mx-auto px-4 py-8 flex-grow">
        ...
        <div class="flex items-center mb-6">
            ...
            <h1 class="text-3xl font-bold text-center mb-8">French Conversation Quiz</h1>
